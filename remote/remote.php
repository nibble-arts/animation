...
// paired sequencer defined
	...
	// ==========================================================================
	// set output dom data
	if ($retQuery->animation ())
		$xml->appendChild ( $xml->createElement ( "animation", $retQuery->animation () ) );
	if ($retQuery->scene ())
		$xml->appendChild ( $xml->createElement ( "scene", $retQuery->scene () ) );
	if ($retQuery->time () >= 0)
		$xml->appendChild ( $xml->createElement ( "time", $retQuery->time () ) );
	if ($retQuery->action ())
		$xml->appendChild ( $xml->createElement ( "action", $retQuery->action () ) );
	if ($retQuery->value ())
		$xml->appendChild ( $xml->createElement ( "value", $retQuery->value () ) );
	if ($retQuery->status ())
		$xml->appendChild ( $xml->createElement ( "status", $retQuery->status () ) );
}
...
class Query {
	...
	function __construct($queryAnimation = "", $queryScene = "", $queryTime = "-1", $queryAction = "", $queryValue = "", $queryStatus = "") {
		if (gettype ( $queryAnimation ) == "array") {
			if (array_key_exists ( "animation", $queryAnimation ))
				$this->queryAnimation = $queryAnimation ["animation"];
			if (array_key_exists ( "scene", $queryAnimation ))
				$this->queryScene = $queryAnimation ["scene"];
			if (array_key_exists ( "time", $queryAnimation ))
				$this->queryTime = $queryAnimation ["time"];
			if (array_key_exists ( "action", $queryAnimation ))
				$this->queryAction = $queryAnimation ["action"];
			if (array_key_exists ( "value", $queryAnimation ))
				$this->queryValue = $queryAnimation ["value"];
			if (array_key_exists ( "status", $queryAnimation ))
				$this->queryStatus = $queryAnimation ["status"];
		} 

		else {
			if ($queryAnimation)
				$this->queryAnimation = $queryAnimation;
			if ($queryScene)
	...
	function get_array() {
		...
		if ($this->queryAnimation)
			$retArray ["animation"] = $this->queryAnimation;
		if ($this->queryScene)
			$retArray ["scene"] = $this->queryScene;
		if ($this->queryTime >= 0)
			$retArray ["time"] = $this->queryTime;
		if ($this->queryAction)
			$retArray ["action"] = $this->queryAction;
		if ($this->queryValue)
			$retArray ["value"] = $this->queryValue;
		if ($this->queryValue)
			$retArray ["status"] = $this->queryStatus;
